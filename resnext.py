...
if __name__ == '__main__':
    ...
    model = ResNext((dr.ROWS, dr.COLS, dr.CHANNELS), depth=47, cardinality=5, width=1, classes = 2)
    model.summary()
    sgd = SGD(lr=0.01, decay=1e-4, momentum=0.9, nesterov=True)
    model.compile(loss='categorical_crossentropy', optimizer=sgd, metrics=['accuracy'])
    for i in range(500):
        model.fit(train, train_labels, batch_size=32, epochs=1)
        scores = model.evaluate(validation, validation_labels, batch_size=148)
